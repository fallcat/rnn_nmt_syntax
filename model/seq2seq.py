...
class BatchEncoderRNN(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        hidden = torch.zeros(self.num_layers, batch_size, self.d_hid, device=DEVICE)
...
class BatchAttnKspanDecoderRNN(nn.Module):
    ...
    def forward(self, inputs, hidden, encoder_outputs):
        ...
        attn_weights = torch.zeros(bsz, span_seq_len)

        for l in range(seq_len):

            attn_weight = F.softmax(self.attn(torch.cat((embeddeds[:, l].contiguous(), hidden), 1)), dim=1)
