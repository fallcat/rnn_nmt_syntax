    ...
    def search_sequential_batch(self, sequences, topv, topi, scores, hiddens, batch_size):
        ...
        for s in range(self.config['span_size']):
            ...
            if s == 0:
                ...
                if s == 1:
                    print("i = 0, j = 0")
                    print("b_matrix prev", b_matrix[a_matrix][0, 0])
                    print("topi[a_matrix, s, colsi] prev", topi[a_matrix, s, colsi][0, 0])
                    print("i = 1, j = 2")
                    print("b_matrix prev", b_matrix[a_matrix][2, 1])
                    print("topi[a_matrix, s, colsi] prev", topi[a_matrix, s, colsi][2, 1])
                    print("i = -1, j = -1")
                    print("b_matrix prev", b_matrix[a_matrix][-1, -1])
                    print("topi[a_matrix, s, colsi] prev", topi[a_matrix, s, colsi][-1, -1])
                b_matrix = torch.cat((b_matrix[a_matrix], topi[a_matrix, s, colsi].to('cpu').unsqueeze(2)), 2)
                b_matrix_list = b_matrix.numpy().tolist()
                lengths = torch.tensor([[len(col) if EOS_token not in col else col.index(EOS_token)
                                         for col in row] for row in b_matrix_list], dtype=torch.float32)
