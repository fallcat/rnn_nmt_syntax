...
class BatchAttnKspanDecoderRNN(nn.Module):
    ...
    def forward(self, inputs, hidden, encoder_outputs):
        ...
        for l in range(seq_len):
            ...
            print("hidden", hidden.size())
            attn_weight = F.softmax(self.attn(torch.cat((embeddeds[:, l].contiguous(), hidden[-1]), 1)), dim=2)
            ...
            output = torch.cat((embeddeds[:, l].unsqueeze(0).repeat(encoder_seq_len, 1, 1), attn_applied), 2)
