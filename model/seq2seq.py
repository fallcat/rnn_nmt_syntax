...
class BatchBahdanauEncoderRNN2(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, dropout_p=0.1, max_length=MAX_LENGTH, rnn_type="GRU", num_directions=1):
        super(BatchBahdanauEncoderRNN2, self).__init__()
        ...
        self.dropout_p = dropout_p
        self.dropout = nn.Dropout(self.dropout_p)
        self.convert = nn.Linear(2 * self.hidden_size, self.hidden_size)
        self.max_length = max_length
...
class BatchBahdanauAttnKspanDecoderRNN4(nn.Module):
    def __init__(self, hidden_size, output_size, num_layers=4, dropout_p=0.1, max_length=MAX_LENGTH, span_size=SPAN_SIZE,
                 rnn_type="GRU", num_directions=1):
        super(BatchBahdanauAttnKspanDecoderRNN4, self).__init__()
        self.hidden_size = hidden_size
        self.output_size = output_size
        self.num_layers = num_layers
        self.dropout_p = dropout_p
        ...
        self.cat_embeddings = nn.Linear(self.hidden_size * self.span_size, self.hidden_size)
        # self.attn = nn.Linear(self.hidden_size * 2, self.max_length)
        self.cat_attn = nn.Linear(self.hidden_size * 2, self.hidden_size * 1)
    ...
    def forward(self, inputs, hidden, cell, encoder_outputs):
        ...
        # attn_weights = F.softmax(
        #     self.attn(torch.cat((embeddeds, hidden[-1]), 1)), dim=1)
        concatted = self.cat_attn(torch.cat((embeddeds, hidden[-1]), 1))
        concatted = torch.cat((
            concatted.unsqueeze(1).expand((concatted.size()[0], encoder_seq_len, concatted.size()[1])),
            encoder_outputs), 2)
        attn_weights = F.softmax(
            torch.chain_matmul(concatted.view(-1, concatted.size()[2]), self.attn, self.v).view(concatted.size()[0],
                                                                                                concatted.size()[1], -1),
            dim=1)

        print("attn_weights", attn_weights.size())
        print("encoder_outputs", encoder_outputs.size())
        attn_applied = torch.bmm(attn_weights.transpose(),
                                 encoder_outputs)
        embeddeds = torch.cat((embeddeds.unsqueeze(1), attn_applied), 2)
        embeddeds = self.attn_combine(embeddeds)
